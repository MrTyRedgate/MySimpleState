-- Flyway MySimpleState Database Setup Script --

-- Drop MySimpleState_dev database if it exists to ensure fresh setup
IF DB_ID('MySimpleState_dev') IS NOT NULL
BEGIN
	USE MASTER
    ALTER DATABASE MySimpleState_dev SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE MySimpleState_dev;
	PRINT 'MySimpleState_dev Database Dropped'
END;

-- Ensure each database exists, creating them if needed
IF DB_ID('MySimpleState_dev') IS NULL CREATE DATABASE MySimpleState_dev PRINT 'MySimpleState_dev Database Created';
IF DB_ID('MySimpleState_test') IS NULL CREATE DATABASE MySimpleState_test PRINT 'MySimpleState_test Database Created';
IF DB_ID('MySimpleState_prod') IS NULL CREATE DATABASE MySimpleState_prod PRINT 'MySimpleState_prod Database Created';
IF DB_ID('MySimpleState_check') IS NULL CREATE DATABASE MySimpleState_check PRINT 'MySimpleState_check Database Created';
IF DB_ID('MySimpleState_build') IS NULL CREATE DATABASE MySimpleState_build PRINT 'MySimpleState_build Database Created';
GO

USE MySimpleState_dev;
GO

ALTER DATABASE MySimpleState_dev
SET MULTI_USER;
GO
...
VALUES ('Flyway Airlines', 'New York', 'London', '2024-11-20 10:00', '2024-11-20 20:00', 500.00, 150),
       ('MySimpleState', 'Los Angeles', 'Tokyo', '2024-12-01 16:00', '2024-12-02 08:00', 800.00, 200);
